...
if __name__=='__main__':
	...
	#SendRobot('test')
	count=os.popen('cat /Users/linjunhao/Desktop/complie_opt/TraversingOutput/APICheck/interface_check.txt|grep 不符合').read()
	count1=os.popen('cat /Users/linjunhao/Desktop/complie_opt/TraversingOutput/APICheck/property_check.txt|grep 不符合').read()
	print count
	if count=='' and count1=='':
		print 'empty'
		SendRobot("API兼容性测试完成，未发现兼容问题")
	else:
		print 'ss'
		SendRobot("API兼容性测试完成，详情数据请查看:http://devops.oa.com/console/pipeline/apiscan/p-f1e62562247947ddb9ddab5f2876a780/detail/%s/partView"%sys.argv[1])
