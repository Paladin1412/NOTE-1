...
def DataInsertIntoTable(line_list):
    if(len(line_list) == 4):
        #sql插入语句
        sql = "INSERT INTO property_data_table(property_name, property_type, interface_name, version_id) \
            VALUES ('%s', '%s', '%s', '%s') " %(line_list[1], line_list[0], line_list[2], line_list[3])
        try:
            #执行sql语句
            cursor.execute(sql)
            #提交到数据库执行
...
if __name__ == '__main__':
    data_dir_path = "/Users/linjunhao/Desktop/complie_opt/TraversingOutput/property"
    sql = "TRUNCATE TABLE property_data_table"
    try:
        cursor.execute(sql)
        db.commit()
        print("property data \n")
